  ...
  'addToGrid': ({ element, grid, visited }) => {
    ...
    if (incoming.length > 1) {
      ...
      if (lowestRow < row && !grid.get(lowestRow, col)) {
        if ((isNextElementExclusiveGatewayOrBoundaryEvent(incoming) && is(element, 'bpmn:ExclusiveGateway'))) {
...
function isNextElementExclusiveGatewayOrBoundaryEvent(elements) {
  return elements.every(element => is(element, 'bpmn:ExclusiveGateway') || is(element, 'bpmn:BoundaryEvent'));
}